...
export default function LandingHero({ onBeginJourney, onOpenTimeline, mousePos }) {
  ...
  const tiltX = (mousePos.y - window.innerHeight / 2) * 0.015;
  const tiltY = (mousePos.x - window.innerWidth / 2) * -0.015;

  return (
    <div className="relative min-h-screen flex flex-col justify-between items-center px-6 py-12 z-20 overflow-hidden">
      ...
      <motion.div
        ...
        className="my-auto text-center max-w-4xl flex flex-col items-center transition-transform duration-300 ease-out"
      >
        {/* Author Tag */}
        <motion.span
        ...
        >
          <button
            ...
            className="group relative px-9 py-4 rounded-full bg-[#f4f1ea] text-[#0a0a0c] font-sans-ui font-medium text-sm tracking-widest uppercase overflow-hidden transition-all duration-500 hover:shadow-[0_0_35px_rgba(244,241,234,0.35)] hover:scale-[1.03] active:scale-[0.98] flex items-center gap-3"
          >
          ...
          <button
            ...
            className="group px-7 py-4 rounded-full glass-card text-stone-300 hover:text-white font-sans-ui font-medium text-sm tracking-widest uppercase transition-all duration-300 flex items-center gap-2.5 hover:border-white/20"
          >
